...
- **Ollama model resolution**: Before the first interview turn and before resuming a dev-plan session, `ScreenController` calls `resolveOllamaModel(currentModel, listModels)` from `src/providers/ollama.ts`. Three outcomes: (1) current model is installed — returned unchanged with no notice; (2) current model is missing or session has no model and other models exist — first available model is auto-selected with a notice when a model was displaced; (3) Ollama has zero installed models — `noModelsInstalled: true`, `interviewStartedRef` is reset, and an actionable notice is shown so the user can install a model and try again. If listing fails (Ollama down), the current model is returned unchanged. For dev-plan resume, if `noModelsInstalled` is true, `setGenerationError` is called instead.
