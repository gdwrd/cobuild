...
src/
  cli/         — entrypoint (index.ts), runtime config (config.ts), startup orchestrator (app-shell.ts:
                 runStartup, StartupResult, StartupStep, StartupProgressCallback,
                 StartupProgressChannel, createStartupProgressChannel)
  ...
  interview/   — interview engine: controller.ts (loop, ModelProvider interface, COMPLETION_MARKER),
                 commands.ts (slash command router, CommandMetadata interface, COMMAND_DEFINITIONS
                 source-of-truth list, KNOWN_COMMANDS, HELP_MESSAGE, filterCommands,
                 buildUnknownCommandMessage), prompts.ts (system prompt, token estimation),
  ...
  providers/   — ollama.ts (OllamaProvider: generate via /api/chat, listModels via /api/tags, 120s timeout;
                 resolveOllamaModel: pre-flight model resolution returning OllamaModelResolution
                 {resolvedModel, noModelsInstalled, notice}),
                 codex-cli.ts (CodexCliProvider: shells out via execFile to the `codex` binary with --quiet,
                 builds a flat text prompt via buildCodexPrompt, 120s timeout),
                 factory.ts (createProvider: constructs OllamaProvider or CodexCliProvider from ProviderName;
                 supportsModelListing: runtime duck-type capability check used by /model handler)
  ...
  ui/          — Ink components: App.tsx (interview transcript view; inputReducer pure reducer for
                 insert/backspace/delete/cursor actions; InputState, InputAction, MAX_VISIBLE_MESSAGES,
                 TranscriptView, InterviewInput, CommandAutocomplete sub-components),
                 AppShell.tsx (shared chrome:
                 status bar, notices, transient errors, footer), ScreenController.tsx (screen router),
                 StartupScreen.tsx (staged startup progress with per-step icons),
                 ErrorScreen.tsx (fatal error display), RestoredSession.tsx (resumed-session interstitial),
  ...
  validation/  — TTY detection and provider readiness checks (env.ts): checkTTY, checkOllama,
                 ...
                 checkProviderReadiness (dispatches to checkOllama or checkCodexCli by ProviderName)
```
...
- **Startup flow**: `src/cli/index.ts` calls `createStartupProgressChannel()` to get `{ channel, onProgress }`, then calls `runStartup(config, onProgress)` to produce `startupPromise`. Both `startupPromise` and `channel` are passed to `ScreenController`. The UI renders `StartupScreen` with per-step progress while startup runs concurrently; `ScreenController` subscribes to `channel` via a `useEffect` and updates `startupSteps` state, which `StartupScreen` uses to render step icons (`✓`, `✗`, spinner). `createStartupProgressChannel` buffers events so late subscribers (mounted after the first `onProgress` call) receive the latest state immediately.
- **Fail-fast validation**: `runStartup` in `app-shell.ts` checks TTY interactivity and directory bootstrap before proceeding — either failure returns `success: false` and exits with code 1. Provider readiness is checked in parallel for all known providers (`checkProviderReadiness`), but provider unavailability does NOT cause exit: it populates `startupNotice` and startup continues normally. Only TTY and bootstrap failures are fatal. For resumed sessions, the provider is determined from the saved session (not from `--provider` flag).
...
- **Screen controller pattern**: `ScreenController.tsx` manages a `Screen` type (`'startup' | 'restored' | 'main' | 'generating' | 'yesno' | 'error' | 'execution'`) and renders different Ink components per state. The `'restored'` screen shows `RestoredSession` when startup resolves with `sessionResolution: 'resumed'`. The `'generating'` screen shows `GenerationScreen` and is entered automatically after the interview completes. The `'yesno'` screen renders `YesNoPrompt` and blocks the artifact pipeline until the user answers; the resolver is stored in `yesNoResolverRef` and invoked by `handleYesNoAnswer`. The `'execution'` screen is dormant (no current code transitions to it) and is reserved for future ralphex runner output.
- **AppShell pattern**: Every screen in `ScreenController` mounts as children of `AppShell` (`src/ui/AppShell.tsx`). `AppShell` renders the persistent status bar (`StatusHeaderData`), a notice area, a transient error area (auto-dismissed after `TRANSIENT_ERROR_DISPLAY_MS = 5000ms`), and a footer help line (`FooterHelpData`). Screen-specific components render only their own content as children — they do not own chrome. Adding a new screen requires implementing a focused view component and mounting it inside `AppShell` with appropriate `statusBar`/`footer` props.
...
- **Global settings precedence**: For new sessions, `runStartup` applies global settings after loading: if `settings.defaultProvider` is set and no `--provider` flag was passed, the default provider is used; if `settings.defaultOllamaModel` is set, it seeds the initial model for Ollama sessions. Resumed sessions always use their saved session provider and model, ignoring both CLI flags and global settings. As a side effect, `/model` saves the chosen model as `defaultOllamaModel` and `/provider` saves the chosen provider as `defaultProvider` in global settings so future new sessions inherit the last explicitly selected values. Both saves are best-effort: failures are logged as warnings and do not abort the command. `RuntimeConfig` also carries `providerExplicit: boolean`, set to `true` only when `--provider` was explicitly passed on the CLI (detected via `program.getOptionValueSource('provider') === 'cli'`); `runStartup` uses this flag to decide whether the CLI flag or the global `defaultProvider` setting wins.
- **Provider-aware model display in header**: `AppShell` never renders a model name for `codex-cli` sessions. The invariant is maintained by always calling `setCurrentModel(undefined)` when the active provider is `codex-cli` (at startup, on provider switch, and on dev-plan resume). Because `currentModel` state is already `undefined` for codex-cli, the `statusBar` construction (`model: currentModel`) naturally passes `undefined`, and `AppShell` provides a second-layer guard (`statusBar.provider !== 'codex-cli' && statusBar.model`) in case the state ever differs. Ollama sessions always show the resolved model. The rule holds across new sessions, resumed sessions, mid-interview `/provider` switches, and dev-plan resume paths.
...
- **`/help` command and unknown command feedback**: `commands.ts` exports `HELP_MESSAGE` (multi-line command reference) and `KNOWN_COMMANDS` (includes `/help`). When `runInterviewLoop` in `controller.ts` encounters an unrecognized slash command it calls `buildUnknownCommandMessage()` and surfaces it as an assistant response — unrecognized commands are no longer silently ignored. `ScreenController` also registers a `helpHandler` in its command router that returns `HELP_MESSAGE` for `/help` invocations during the interview.
- **ModelSelectPrompt interaction flow**: When `/model` is invoked and the provider supports model listing, `ScreenController` calls `onSelectModel` which sets `modelSelectOptions` state and flips `isSelectingModelRef`. This mounts `ModelSelectPrompt` in place of the normal interview input, suspending the interview loop. The user's raw input is resolved via `onUserInput()` (the same promise-based mechanism as the interview). `allowEmptySubmit: true` is set during selection so pressing Enter with no input keeps the current model. After resolution, `ModelSelectPrompt` is unmounted and the interview resumes.
...
- **Session schema versioning**: Sessions carry `schemaVersion: number` (currently `2`, exported as `CURRENT_SCHEMA_VERSION`). Every `loadSession()` call passes the raw JSON through `migrateSession()`, which back-fills missing optional fields and logs when upgrading from an older version. Sessions without `schemaVersion` are treated as version 0 and migrated silently.
- **Provider system**: `ProviderName = 'ollama' | 'codex-cli'` is defined in `session.ts` and used throughout. `RuntimeConfig` carries `provider: ProviderName` (defaults to `'ollama'`; set from `--provider` flag which now uses Commander `.choices()` to reject unknown values). Sessions store `provider?: ProviderName`; old sessions without this field default to `'ollama'` during migration and startup. Passing `--provider` is silently ignored when resuming an existing session — the saved session's provider takes precedence. To switch providers on an existing directory, pass `--new-session --provider <name>`. Neither `createProvider` nor `/provider ollama` inject a fallback model name — `OllamaProvider` may be constructed with `model: undefined`; the concrete model is resolved at runtime by `resolveOllamaModel` in `ScreenController` before the first interview turn.
- **Ollama model resolution**: Before the first interview turn and before resuming a dev-plan session, `ScreenController` calls `resolveOllamaModel(modelHint, listModels)` from `src/providers/ollama.ts`. The hint passed is `currentModelRef.current ?? globalSettingsRef.current?.defaultOllamaModel` — so the priority is: saved session model > global settings default > first available model. Three outcomes: (1) hint model is installed — returned unchanged with no notice; (2) hint model is missing or absent and other models exist — first available model is auto-selected with a notice when a model was displaced; (3) Ollama has zero installed models — `noModelsInstalled: true`, `interviewStartedRef` is reset, and an actionable notice is shown so the user can install a model and try again. If listing fails (Ollama down), the hint model is returned unchanged. For dev-plan resume, if `noModelsInstalled` is true, `setGenerationError` is called instead. `StartupResult` carries `globalSettings?: GlobalSettings`; `ScreenController` stores it in `globalSettingsRef` to make the default model available at resolution time.
- **Interview input reducer**: `App.tsx` manages input buffer and cursor position via `useReducer(inputReducer, INITIAL_INPUT_STATE)`. `inputReducer` is a pure function (exported and unit-tested directly) that handles `insert`, `backspace`, `delete`, `left`, `right`, `home`, `end`, and `clear` actions. `inputStateRef` mirrors committed state so rapid-keypress sequences inside `useInput` always see the latest buffer. `isThinkingRef` mirrors the `isThinking` prop for the same reason — both refs are synced in an effect that runs after every render.
- **Slash command autocomplete**: When the input buffer starts with `/` and contains no spaces, `App.tsx` calls `filterCommands(value)` from `commands.ts` and renders `CommandAutocomplete` above the input. `autocompleteIndex` (state, 0-based) tracks the highlighted entry and is kept in sync via `autocompleteIndexRef` for use inside `useInput`. Up/Down arrows adjust the index; Enter with autocomplete open submits the highlighted command's `name` rather than the raw buffer; Escape clears the buffer whenever it starts with `/` (regardless of whether any suggestions are visible). The index resets to 0 on every value change or `isThinking` state change.
