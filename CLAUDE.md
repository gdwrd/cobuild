...
- **Fail-fast validation**: `runStartup` in `app-shell.ts` checks TTY interactivity, provider readiness (Ollama reachability or Codex CLI binary availability via `checkProviderReadiness`), and directory bootstrap before proceeding. Any failure causes the UI to display an error and exit with code 1. For resumed sessions, the provider is determined from the saved session (not from `--provider` flag).
